...
(defn register! [{:keys [session]} user]
  (if-let [error (registration-errors user)]
    (response/precondition-failed {:result :error :message (first (:pass error))})
    (try
      (db/create-user!
       (-> user
           (dissoc :pass-confirm)
...
;; I think we change
;; (if (registration-errors user) (response/precondition-failed {:result :error})
;; ->
;; (if-let [error (registration-errors user)]
;;   (response/precondition-failed {:result :error :message (first (:pass error))})
